...
/* Primeval Refuge Gateway Live Map + Guide v0.13.0 */
...
function filterGuideCards(){const q=String($("guideSearch")?.value||"").trim().toLowerCase(),cards=[...document.querySelectorAll(".guide-card")],symbols=[...document.querySelectorAll(".guide-symbol-item")];let shown=0;cards.forEach(card=>{const hay=((card.dataset.guideTags||"")+" "+card.textContent).toLowerCase(),ok=!q||hay.includes(q);card.hidden=!ok;if(ok){shown++;if(q&&card.tagName==="DETAILS")card.open=true;}});symbols.forEach(item=>{const hay=((item.dataset.guideTags||"")+" "+item.textContent).toLowerCase(),ok=!q||hay.includes(q);item.hidden=!ok;});const grid=$("guideGrid");if(grid)grid.classList.toggle("no-results",shown===0);}
